    ...
    return Promise.try(function () {
        ...
        return self.client.fetchRequest(data).map(function (p) {
            ...
            if (p.messageSet.length) {
              ...
              if (typeof s.syncHandler === 'function') {
                s.syncHandler(p.topic, p.partition, s.offset)
              }
            }
            return null;
...
BaseConsumer.prototype._updateSubscription = function (topic, partition) {
    ...
    return self.client.updateMetadata().then(function () {
        ...
        return self.subscribe(topic, partition, s.offset !== undefined ? { offset: s.offset } : s.options, s.handler, s.syncHandler)
        .catch(function (err) {
 ...
 * @return {Promise}
 */
BaseConsumer.prototype.subscribe = function (topic, partitions, options, handler, syncHandler) {
    var self = this;
